...
#define TAG "bsdiff-jni" // 这个是自定义的LOG的标识
...
Java_com_tyland_tool_DiffPatchUtil_diff(JNIEnv *env, jclass type, jstring oldPath_,
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_tyland_tool_DiffPatchUtil_patch(JNIEnv *env, jclass type, jstring oldPath_,
                                                     jstring newPath_, jstring patchPath_) {
    const char *oldPath = env->GetStringUTFChars(oldPath_, 0);
    const char *newPath = env->GetStringUTFChars(newPath_, 0);
    const char *patchPath = env->GetStringUTFChars(patchPath_, 0);
...
Java_com_tyland_tool_DiffPatchUtil_diffStr(JNIEnv *env, jclass type, jstring oldStr_,
    ...
    for (int i = 0; i < result; ++i) {
        ...
        jclass jcBsdiffBean = env->FindClass("com/tyland/tool/BsdiffBean");
